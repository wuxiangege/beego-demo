...
type News struct {
	Cid         int       `orm:"column(cid);null" description:"新闻分类"`
	Content     string    `orm:"column(content);null" description:"新闻内容"`
	CreatedTime time.Time `orm:"column(created_time);type(date);null" description:"新闻发布时间"`
...
func Ping() error {
	o := orm.NewOrm()
	_, err := o.Raw("UPDATE `news` SET `title` = ? WHERE `id` = ?", "555", "1").Exec()
	return err
}
